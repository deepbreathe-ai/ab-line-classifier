...
def create_ABline_dataframe(database_query, update):
    '''
    ...
    df['Path'] = df.apply(lambda row: '../../data/masked_clips/' + row.filename, axis=1)
    ...
    df.to_csv(cfg['PATHS'][CSV_PATH], index=False)
    return df, update

if __name__ == "__main__":
    create_ABline_dataframe(database_query, 1)
