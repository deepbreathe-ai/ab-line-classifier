...
def keras_to_frozen_pb(model_in_path,
    ...
    with graph.as_default():
        ...
        # model = tf.lite.TFLiteConverter.from_keras_model(model_in_path)
        # load the model to graph and sess
        model = tf.keras.models.load_model(cfg['PATHS']['MODEL_TO_LOAD'], compile=False)
        ...
        tf.io.write_graph(graphdef, './', model_out_path, as_text=False)
...
if __name__ == "__main__":
    ...
    input_keras_model = "results/models/model_dummy.h5"
    output_pb_model = "results/models/model_dummy.pb"
